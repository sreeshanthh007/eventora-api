...
let AdminLoginStrategy = class AdminLoginStrategy {
    constructor(passwordBcrypt, adminRepository) {
        this.passwordBcrypt = passwordBcrypt;
        this.adminRepository = adminRepository;
    }
    login(user) {
        return __awaiter(this, void 0, void 0, function* () {
            const admin = yield this.adminRepository.findByEmail(user.email);
            if (!admin) {
                throw new custom_error_1.CustomError(constants_1.ERROR_MESSAGES.EMAIL_NOT_FOUND, constants_1.HTTP_STATUS.NOT_FOUND);
            }
            if (user.password) {
                const isPasswordMatch = yield this.passwordBcrypt.compare(user.password, admin.password);
                if (!isPasswordMatch) {
...
exports.AdminLoginStrategy = AdminLoginStrategy = __decorate([
    ...
    __param(1, (0, tsyringe_1.inject)("IAdminRepository")),
