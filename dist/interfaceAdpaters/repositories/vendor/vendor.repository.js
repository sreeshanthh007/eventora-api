...
let VendorRepository = class VendorRepository {
    ...
    findPaginatedClients(filter, skip, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            const [user, total] = yield Promise.all([
                vendor_model_1.VendorModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit),
                vendor_model_1.VendorModel.countDocuments(filter),
            ]);
            return {
                user,
                total,
    ...
    findPaginatedVendorByStatus(filter, skip, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            const [vendors, total] = yield Promise.all([
                vendor_model_1.VendorModel.find(filter)
                    .sort({ createdAt: -1 })
                    .skip(skip)
                    .limit(limit),
                vendor_model_1.VendorModel.countDocuments(filter)
